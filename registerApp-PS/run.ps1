...
if (!($appExist)) {
    ...
    Connect-AzureRmAccount -ServicePrincipal -Credential $credential -TenantId $tenantid -Subscription "2f230ac3-94ea-48b6-93b9-8b1fc97add0a"
    ...
    $keyvault = New-AzureRmKeyVault -name $($applicationName -Replace('[\W]','')) -ResourceGroupName AzureSelfServiceKeyVault -Location "North Europe"
    Set-AzureRmKeyVaultAccessPolicy -VaultName $($applicationName -Replace('[\W]','')) -PermissionsToSecrets set -ServicePrincipalName $ApplicationId

    # skriver app secret til Azure keyvault
    $vaultSecret = Set-AzureKeyVaultSecret -VaultName $($applicationName -Replace('[\W]','')) -name $($applicationName -Replace('[\W]','')) -SecretValue $secret
    ...
    foreach ($owner in $owners)
    {
        ...
        Set-AzureRmKeyVaultAccessPolicy -VaultName $($applicationName -Replace('[\W]','')) -PermissionsToSecrets get,list -UserPrincipalName $owner
    }
